...
def crear_curso(tipo):
    ...
    match tipo.upper():
        case "O":
            plataformacurso = input("Indica la plataforma en la que se hara el curso:\n")
...
def agregar_curso(tipo, curso_creado):
    match tipo.upper():
        case "O":
            cursos.update({curso_creado.codigo : {"titulo":curso_creado.titulo,"horas":curso_creado.horas,"precio":curso_creado.precio}})
        case "P":
            cursos.update({curso_creado.codigo : {"titulo":curso_creado.titulo,"horas":curso_creado.horas,"precio":curso_creado.precio}})
...
def mostrar_cursos():
    for key,value in cursos.items():        
        print(f"Codigo:{key} | Titulo:{value}")
salir=True
opciones={
    "A":"Añadir curso",
...
for i in listacursos:
    cursos.update({i.codigo : {"titulo":i.titulo,"horas":i.horas,"precio":i.precio}})
